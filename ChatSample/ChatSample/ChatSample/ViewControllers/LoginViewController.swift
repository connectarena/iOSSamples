...
class LoginViewController: UIViewController {
    ...
    @IBAction func NextAction(_ sender: UIBarButtonItem) {
        ...
        guard self.loginID.text?.count != 0 else {
            ...
            Util().showAlert(withTitle: "ChatSample", andMessage: "Please enter Login ID", onView: self)
            return
        }
        
        guard self.password.text?.count != 0 else {
            Util().showAlert(withTitle: "ChatSample", andMessage: "Please enter password", onView: self)
            return
        }
        ...
        CSClient.sharedInstance()?.login(self.loginID.text!, withPassword: self.password.text!, completionHandler: { response , error in
            ...
            if error != nil {
                ...
                case .E_401_UNAUTHORIZED:
                    ...
                    Util().showAlert(withTitle: "ChatSample", andMessage: "Login Failed : Unauthorized", onView: self)

                case .E_409_NOTALLOWED:
                    
                    Util().showAlert(withTitle: "ChatSample", andMessage: "Login Failed : Not allowed", onView: self)

                case .E_803_USER_DEACTIVATED:
                    
                    Util().showAlert(withTitle: "ChatSample", andMessage: "Login Failed : User deactivated", onView: self)
